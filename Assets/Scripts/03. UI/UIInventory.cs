...
public class UIInventory : MonoBehaviour
{
    ...
    public void SelectItem(int index)
    {
        ...
        // SelectedItem = ItemSlot /     Slots : 배열
        // 나의경우 아이템 슬롯으로 속성이 되어있음, 여기 스크립트 내에서
    ...
    {
        PlayerController player = CharacterManager.Instance.Player.GetComponent<PlayerController>();
        ...
        if (selectedItem.item.type == ItemType.Consumable)
        {
            ...
            for (int i = 0; i < selectedItem.item.timeconsumable.Length; i++)
            {
                player.ApplyTemporaryEffect(selectedItem.item.timeconsumable[i]);
            }
            RemoveSelctedItem();
        }
    }
